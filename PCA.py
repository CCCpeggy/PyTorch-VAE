...
if "__main__" == __name__:
    ...
    experiment = VAEXperiment(model, config['exp_params'], select_rect=True)
    dataloader = experiment.predict_dataloader()

    global_exp_params = config['exp_params']
    global_exp_params['data_path'] = global_path
    global_experiment = VAEXperiment(model, global_exp_params, select_rect=False)
    ...
    db = AgglomerativeClustering(n_clusters=25).fit(local_pca_z)
    labels = db.labels_
    ...
    for l in label_all:
        ...
        dist, ind = dist[0][0], ind[0][0]
        folder = os.path.basename(global_file_name[ind])[:-4]

        os.rename(os.path.join(classify_dir, str(l)), os.path.join(classify_dir, str(l) + "_" + folder))
