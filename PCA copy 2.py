...
if "__main__" == __name__:
    ...
    global_dataloader = global_experiment.val_dataloader()
    print("End")
    first = True
    total_z = None
    total_image = None
    ...
    for image, path in global_dataloader:
        ...
        if global_count > local_count*10:
            break
        print(global_count)
    ...
    db = AgglomerativeClustering(n_clusters=50).fit(z_pca)
    labels = db.labels_
    ...
    for i in range(local_count):
        ...
        cv2.imwrite(os.path.join(cut_dir, str(labels[i]), "local_{}.png".format(i)), np.moveaxis(total_image[i], 0, -1)*255)
    for i in range(global_count):
        ii = local_count + i
        cv2.imwrite(os.path.join(cut_dir, str(labels[ii]), "global_{}.png".format(i)), np.moveaxis(total_image[ii], 0, -1)*255)
